...
def load_data_to_dim_schedules_table(postgres_connection):
    try:
        ...
        src_table_name                  =   'dim_flight_schedules_tbl'
        table_name                      =   'dim_schedules_tbl'
        ...
        # Pull flight_schedules_tbl data from dwh tables in Postgres database 
        try:
            fetch_dim_flight_schedules_tbl = f'''     SELECT { ', '.join(desired_sql_columns) } FROM {active_schema_name}.{src_table_name};  
            '''
            root_logger.debug(fetch_dim_flight_schedules_tbl)
            root_logger.info("")
            ...
            cursor.execute(fetch_dim_flight_schedules_tbl)
        ...
        create_dim_flight_schedules_tbl = f'''                CREATE TABLE IF NOT EXISTS {active_schema_name}.{table_name}  AS
                                                                        SELECT 
                                                                                    flight_sk as flight_id,
                                                                                    flight_id as flight_old_id,
                                                                                    arrival_city,
                                                                                    arrival_date,
                                                                                    arrival_time,
                                                                                    departure_city,
...
load_data_to_dim_schedules_table(postgres_connection)
