...
def load_data_to_dim_flight_promotion_deals_table(postgres_connection):
    try:
        ...
        src_table_name                  =   'stg_flight_promotion_deals_tbl'
        table_name                      =   'dim_flight_promotion_deals_tbl'
        ...
        # Pull flight_promotion_deals_tbl data from semantic tables in Postgres database 
        try:
            fetch_stg_flight_promotion_deals_tbl = f'''     SELECT { ', '.join(desired_sql_columns) } FROM {active_schema_name}.{src_table_name};  
            '''
            root_logger.debug(fetch_stg_flight_promotion_deals_tbl)
            root_logger.info("")
            ...
            cursor.execute(fetch_stg_flight_promotion_deals_tbl)
        ...
        create_dim_flight_promotion_deals_tbl = f'''                CREATE TABLE IF NOT EXISTS {active_schema_name}.{table_name}  (
                                                                                flight_promotion_deal_sk            SERIAL PRIMARY KEY,
...
load_data_to_dim_flight_promotion_deals_table(postgres_connection)
