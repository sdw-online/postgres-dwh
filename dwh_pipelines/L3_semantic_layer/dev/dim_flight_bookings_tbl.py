...
def load_data_to_dim_flight_bookings_table(postgres_connection):
    try:
        ...
        src_table_name                  =   'stg_flight_bookings_tbl'
        table_name                      =   'dim_flight_bookings_tbl'
        ...
        # Pull flight_bookings_tbl data from semantic tables in Postgres database 
        try:
            fetch_stg_flight_bookings_tbl = f'''     SELECT { ', '.join(desired_sql_columns) } FROM {active_schema_name}.{src_table_name};  
            '''
            root_logger.debug(fetch_stg_flight_bookings_tbl)
            root_logger.info("")
            ...
            cursor.execute(fetch_stg_flight_bookings_tbl)
        ...
        create_dim_flight_bookings_tbl = f'''                CREATE TABLE IF NOT EXISTS {active_schema_name}.{table_name}  (
                                                                                flight_booking_sk       SERIAL PRIMARY KEY,
                                                                                flight_booking_id       UUID NOT NULL UNIQUE,
...
load_data_to_dim_flight_bookings_table(postgres_connection)
