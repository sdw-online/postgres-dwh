...
def load_data_to_dim_flight_destinations_table(postgres_connection):
    try:
        ...
        src_table_name                  =   'stg_flight_destinations_tbl'
        table_name                      =   'dim_flight_destinations_tbl'
        ...
        # Pull flight_destinations_tbl data from semantic tables in Postgres database 
        try:
            fetch_stg_flight_destinations_tbl = f'''     SELECT { ', '.join(desired_sql_columns) } FROM {active_schema_name}.{src_table_name};  
            '''
            root_logger.debug(fetch_stg_flight_destinations_tbl)
            root_logger.info("")
            ...
            cursor.execute(fetch_stg_flight_destinations_tbl)
...
load_data_to_dim_flight_destinations_table(postgres_connection)
